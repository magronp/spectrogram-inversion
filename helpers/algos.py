...
def amplitude_mask(spectro_mag, mix_stft, win_length=None, hop_length=None, win_type='hann'):
    ...
    # inverse STFT
    src_est = istft(stft_est, hop_length=hop_length, win_length=win_length, window=win_type)
...
def misi(mix_stft, spectro_mag, win_length=None, hop_length=None, src_ref=None, max_iter=20, win_type='hann'):
    ...
    for iteration_number in range(max_iter):
        # STFT
        stft_est = stft(src_est, n_fft=n_fft, hop_length=hop_length, win_length=win_length, window=win_type)
        ...
        # Inverse STFT
        src_est = istft(stft_est, win_length=win_length, hop_length=hop_length, window=win_type)
        # BSS score
        if compute_sdr:
            sdr.append(get_score(src_ref, src_est))
        # Error
...
def p_cons(sources_stft, win_length, hop_length=None, window='hann'):
    ...
    update_sources_stft = stft(estimated_sources, n_fft=n_fft, hop_length=hop_length, win_length=win_length, window=window)
    return update_sources_stft


def p_mix(sources_stft, mixture_stft, mixing_weights=None):
    """Projector on the conservative subspace, which consists in calculating the mixing error and distributing it over the components
    """
...
def spectrogram_inversion_update(sources_stft, mixture_stft, target_magnitudes, win_length, algo='MISI',
                                 consistency_weigth=1, mixing_weights=None, hop_length=None, window='hann',
                                 compute_error=False):
    ...
    if algo == 'MISI':
        ...
        update_sources_stft = p_mix(aux_cons, mixture_stft)
        update_sources_stft = p_mag(update_sources_stft, target_magnitudes)
        # Error
        if compute_error:
